...
for scenario_number in range(0,No_scenarios):
    ...
    plts = glob.glob(scenario + 'APD.plt')
    No_runs = len(plts)
    data = []
...
fig, (ax1, ax2, ax3)=plt.subplots(3, sharey='all')
...
ax1.set(xlabel='Simulation time (s)', ylabel='APD (ms)')
...
ax2.set(xlabel='Simulation time (s)', ylabel='APD (ms)')
...
ax3.set(xlabel='Simulation time (s)', ylabel='APD (ms)')
...
fig.savefig('lineplot_APD.pdf', dpi=1000, bbox_inches='tight', bbox_extra_artists=legends)

extent = ax1.get_tightbbox(fig.canvas.renderer).transformed(fig.dpi_scale_trans.inverted())
fig.savefig('lineplot_APD_100U.pdf', bbox_inches=extent.expanded(1.02, 1.05))

extent = ax2.get_tightbbox(fig.canvas.renderer).transformed(fig.dpi_scale_trans.inverted())
fig.savefig('lineplot_APD_200U.pdf', bbox_inches=extent.expanded(1.02, 1.05))

extent = ax3.get_tightbbox(fig.canvas.renderer).transformed(fig.dpi_scale_trans.inverted())
fig.savefig('lineplot_APD_300U.pdf', bbox_inches=extent.expanded(1.02, 1.05))
